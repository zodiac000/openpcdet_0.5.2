...
distance_threshold = 60  # 只考虑这个距离内的gt和pred
...
def clean_data_by_distance(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if difficulty == 0:
            if (dis > MAX_DISTANCE[difficulty]):
                ignore = True
        elif ((dis > MAX_DISTANCE[difficulty]) or \
    ...
    for i in range(num_dt):
        ...
        if difficulty == 0:
            if dis > MAX_DISTANCE[difficulty]:
                ignore = True
        elif dis > MAX_DISTANCE[difficulty] or dis <= MAX_DISTANCE[difficulty - 1]:
            ignore = True
    ...
    for i in range(num_gt):
        ...
        if difficulty == 0:
            if ((gt_anno["occluded"][i] > MAX_OCCLUSION[2])
                    ...
                    or (height <= MIN_HEIGHT[2])) or num_point_gt > MAX_POINTS[difficulty]:
                ignore = True
        elif ((gt_anno["occluded"][i] > MAX_OCCLUSION[2])
                or (gt_anno["truncated"][i] > MAX_TRUNCATION[2])
                or (height <= MIN_HEIGHT[2]) or (num_point_gt > MAX_POINTS[difficulty])
                    or (num_point_gt <= MAX_POINTS[difficulty - 1])):
            # if gt_anno["difficulty"][i] > difficulty or gt_anno["difficulty"][i] == -1:
            ignore = True
        if valid_class == 1 and not ignore:
            ignored_gt.append(0)
...
def do_eval(gt_annos,
            ...
            compute_aos=False,
            PR_detail_dict=None,
            DIForDIS=True,  # 为True，则不进行分段处理
            Points=False):  # 为False，则不进行对点数分段处理
...
def get_distance_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    ret_dict = {}
    ret_dict["mAPbev"] = mAPbev
    ret_dict["mAP3d"]  = mAP3d
    ret_dict["mAPbev_R40"] = mAPbev_R40
    ret_dict["mAP3d_R40"]  = mAP3d_R40
    ret_dict["class_to_name"]  = class_to_name    
    
    print("**************** Points interval result: ********************")
    print("max_points: {}".format(MAX_POINTS))
    print("mapbev: {}, map3d:{}, mapbev_R40:{}, map3d_R40:{}".format(mAPbev,mAP3d,mAPbev_R40,mAP3d_R40))
    return result, ret_dict
...
def get_points_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            ...
            break
    mAPbev, mAP3d, mAPbev_R40, mAP3d_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, Points=True)
...
def plot_dist_hist(mAP3d_R40, class_to_name, save_dir):
    for cls in range(mAP3d_R40.shape[0]):          
        plt.figure()
        x_shift_1 = [x-2.5 for x in MAX_DISTANCE]
        x_shift_2 = [x+2.5 for x in MAX_DISTANCE]
        plt.bar(x_shift_1, mAP3d_R40[cls,:,0], alpha=0.9, width = 5, facecolor = 'lightskyblue', edgecolor = 'white', label='one', lw=1)
        plt.bar(x_shift_2, mAP3d_R40[cls,:,1], alpha=0.9, width = 5, facecolor = 'yellowgreen', edgecolor = 'white', label='one', lw=1)
        plt.title("%s distance mapR40"%class_to_name[cls])
        plt.legend(["iou: 0.7","iou: 0.5"])
        plt.savefig("%s/%d_dis.png"%(save_dir,cls))
...
def debug_eval():
    ...
    hist_dir = "%s/../"%pred_dir
    dist_map_file = "%s/dist_map.pkl"%hist_dir
    class_names = ['Car', 'Pedestrian', 'Cyclist']

    if not os.path.exists(dist_map_file) or 1:
        ...
        # ap_result_str, ap_dict = get_points_eval_result(gt_annos, dt_annos, class_names)

        with open("%s/dist_map.pkl"%hist_dir, "wb") as fid:
            pickle.dump(ap_dict, fid)
    else:
        with open("%s/dist_map.pkl"%hist_dir, "rb") as fid:
            ap_dict = pickle.load(fid)
        with np.printoptions(precision=3, suppress=True):
            print("mapbev_R40:{}, map3d_R40:{}".format(ap_dict["mAPbev_R40"].astype(np.float32), ap_dict["mAP3d_R40"].astype(np.float32)))
    ...
    plot_dist_hist(ap_dict["mAP3d_R40"], ap_dict["class_to_name"], hist_dir_3d)
    plot_dist_hist(ap_dict["mAPbev_R40"], ap_dict["class_to_name"], hist_dir_bev)
