...
from ...utils import box_utils, common_utils, object3d_pandaset
from ..dataset import DatasetTemplate


class LidarPandasetDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if not root_path:
            self.set_split2(split)  # 训练时调用
            print("after split, data_paths: ", self.root_paths)

        self.lidar_pandaset_infos = []
        self.include_lidar_pandaset_data(self.mode)
        self.pass_root_path = root_path  # 传入的root_path，生成pickle时调用

    def set_split2(self, split):
        """
        训练时调用。不调基类的__init__，和set_split做区分。因为训练时如果再调用__init__，会调两遍，数据会double
        :param split:
        :return:
        """
        ...
        if len(self.root_paths) > 1:
            for root_idx, root_p in enumerate(self.root_paths):
                ...
                self.sample_id_list.append(sample_idx_list)  # TODO: (du) 需要sorted吗？不排序好像没问题。
        else:
    ...
    def include_lidar_pandaset_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading PandaSet dataset')
        lidar_pandaset_infos = []
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            for root_p in self.root_paths:
                ...
                with open(infos_path, 'rb') as f:
                    infos = pickle.load(f)
                    lidar_pandaset_infos.extend(infos)

        self.lidar_pandaset_infos.extend(lidar_pandaset_infos)

        if self.logger is not None:
            self.logger.info('Total samples for UNTOUCH dataset: %d' % (len(lidar_pandaset_infos)))

    def set_split(self, split):
        """
        生成pkl时调用。仅支持单个root_path。
        :param split:
        :return:
        """
        super().__init__(
            dataset_cfg=self.dataset_cfg, class_names=self.class_names,
            training=self.training, root_path=self.pass_root_path,
            logger=self.logger
        )
        print("calling set_split: {}".format(split))
        self.split = split
        split_flag = None
        if self.split == 'train':
            split_flag = 'training'
        ...
        # else:
        self.root_split_paths.append(os.path.join(self.pass_root_path, split_flag))
        label_dir = os.path.join(self.root_paths[0], split_flag, 'label')
        self.labels_dir.append(label_dir)
        label_files = os.listdir(label_dir)
        self.lidar_dir.append(os.path.join(self.root_paths[0], split_flag, 'velodyne'))
    ...
    def get_lidar(self, list_index, sample_name):
        ...
        return np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 4)  # 注意这里加载了4维
    ...
    def get_label(self, list_index, sample_name):
        ...
        return object3d_pandaset.get_objects_from_label(label_file)  # 注意这里调用的是pandaset的类别！！

    def get_road_plane(self, list_index, sample_name):
        plane_file = os.path.join(self.root_split_paths[list_index], 'planes', "%s.txt"%sample_name)
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_name):
            ...
            if has_label:
                ...
                annotations = {}
                # TODO: (du) 多个数据集联合训练时，请提前将标签中的类别统一。
                ...
                if count_inside_pts:
                    ...
                    num_points_in_gt = -np.ones(num_gt, dtype=np.int32)  # 用-1初始化
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        :return:
        """
        if 'annos' not in self.lidar_pandaset_infos[0].keys():
            return None, {}

        from .lidar_object_eval_python import eval as lidar_eval

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.lidar_pandaset_infos]

        ap_result_str, ap_dict = lidar_eval.get_official_eval_result(eval_gt_annos, eval_det_annos, class_names)
        print(ap_result_str)
        distance_ap_result_str, distance_ap_dict = lidar_eval.get_distance_eval_result(eval_gt_annos, eval_det_annos, class_names)
        point_ap_result_str, point_ap_dict = lidar_eval.get_points_eval_result(eval_gt_annos, eval_det_annos, class_names)

        return ap_result_str, ap_dict

    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.lidar_pandaset_infos) * self.total_epochs

        return len(self.lidar_pandaset_infos)

    def __getitem__(self, index):
        # index = 4
        if self._merge_all_iters_to_one_epoch:
            index = index % len(self.lidar_pandaset_infos)

        info = copy.deepcopy(self.lidar_pandaset_infos[index])
        ...
        if 'annos' in info:
            ...
            road_plane = self.get_road_plane(dir_index, sample_idx)
...
def create_lidar_pandaset_infos(dataset_cfg, class_names, data_path, save_path, workers=4, split='train'):
    dataset = LidarPandasetDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, test_split = "train", "test"
    train_filename = os.path.join(save_path, ('untouch_infos_%s.pkl' % train_split))
    test_filename = os.path.join(save_path, 'untouch_infos_%s.pkl'% test_split)
    ...
    print('untouch info {} file is saved to %{}'.format(train_split, train_filename))

    dataset.set_split(test_split)
    lidar_pandaset_infos_test = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(test_filename, "wb") as f:
        pickle.dump(lidar_pandaset_infos_test, f)
    print("untouch info {} file is saved to %{}".format(test_split, test_filename))
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_lidar_pandaset_infos':
        import yaml
        from pathlib import Path
        ...
        Data_DIR = Path("/data9/duzhe/dataset/opendata/pandaset")
        create_lidar_pandaset_infos(
