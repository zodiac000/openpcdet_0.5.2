...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None, configs=None):
        ...
        for cur_cfg in aug_config_list:
            if not isinstance(augmentor_configs, list):
                if cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST:
                    continue
            cur_augmentor = getattr(self, cur_cfg.NAME)(config=cur_cfg)  # 会从这里就直接进入各个增广方式
    ...
    def random_local_dropout(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_local_dropout, config=config)
        prob_range = config['PROB_RANGE']
        prob = config['PROB']
        gt_boxes, points = data_dict['gt_boxes'], data_dict['points']
        # print("before dropout: ", len(points))
        gt_boxes, points = augmentor_utils.random_local_points_dropout(gt_boxes, points, prob_range=prob_range, prob=prob)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        # print("after dropout: ", len(points))
